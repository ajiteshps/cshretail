                ...
                if (accountObject.rowCount != undefined && accountObject.rowCount > 0) {
                    ...
                    if (account_ids.length > 0) {
                        ...
                        if (req.query.type == 'Retailer') {
                            ...
                            accountOrderSql = `SELECT DISTINCT on (dealer__c) dealer__c ,name,sfid,date_part('epoch'::text, Order_Date__c) * (1000)::double precision as Order_Date__c,Retailer__c,Total_Product_SKU__c,order_value__c FROM ${process.env.TABLE_SCHEMA_NAME}.Order__c where Dealer__c AND Dealer__c.Dealer = OEM AND Dealer__c.Dealer = OEM AND Dealer__c.Dealer = Customer_Sales_Service AND Dealer__c.Dealer = Dealers_Sales_Service IN ('${account_ids}') order by dealer__c,createddate desc`;
                        }
