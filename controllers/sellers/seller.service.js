            ...
            if (teamDetail.success) {
                ...
                if (accountObject.rowCount != undefined && accountObject.rowCount > 0) {
                    ...
                    if (account_ids.length > 0) {
                        ...
                        if (req.query.type == 'Retailer') {
                            accountOrderSql = `SELECT DISTINCT on (retailer__c) retailer__c ,name,sfid,date_part('epoch'::text, Order_Date__c) * (1000)::double precision as Order_Date__c,dealer__c,Total_Product_SKU__c,order_value__c FROM ${process.env.TABLE_SCHEMA_NAME}.Order__c where Retailer__c.Record_Type_Developer_Name__c = Retailers_Sales_Service IN ('${account_ids}') order by retailer__c,createddate desc`;

                            //OR Retailer__c.Record_Type_Developer_Name__c = Customer_Sales_Service OR Retailer__c.Record_Type_Developer_Name__c = Dealers_Sales_Service OR Retailer__c.Record_Type_Developer_Name__c = Retailers_Sales_Service
                         }
                         else
                          {
