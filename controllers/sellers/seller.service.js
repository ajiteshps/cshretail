            ...
            if (teamDetail.success) {
                ...
                if (accountObject.rowCount != undefined && accountObject.rowCount > 0) {
                    ...
                    if (account_ids.length > 0) {
                        ...
                        if (req.query.type == 'Retailer') {
                            accountOrderSql = `SELECT DISTINCT on (retailer__c) retailer__c ,name,sfid,date_part('epoch'::text, Order_Date__c) * (1000)::double precision as Order_Date__c,dealer__c,Total_Product_SKU__c,order_value__c FROM ${process.env.TABLE_SCHEMA_NAME}.Order__c where Retailer__c.Retailer = ASC AND Retailer__c.Retailer = Customer Sales Service AND Retailer__c.Retailer = Dealers Sales Service AND Retailer__c.Retailer = Retailers Sales Service IN ('${account_ids}') order by retailer__c,createddate desc`;
                        } else {
                            accountOrderSql = `SELECT DISTINCT on (dealer__c) dealer__c ,name,sfid,date_part('epoch'::text, Order_Date__c) * (1000)::double precision as Order_Date__c,Retailer__c,Total_Product_SKU__c,order_value__c FROM ${process.env.TABLE_SCHEMA_NAME}.Order__c where Dealer__c.Dealer = OEM AND Dealer__c.Dealer = OEM AND Dealer__c.Dealer = Customer Sales and Service AND Dealer__c.Dealer = Dealers Sales Service    IN ('${account_ids}') order by dealer__c,createddate desc`;
                        }
