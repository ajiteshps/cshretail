...
async function addContact(myDetails,contactObj){
    try{
        ...
        if (myDetails.rowCount > 0) {
            ...
            contactFields =`firstname,lastname,pg_id__c,asm__c,area__c,attached_dealer__c,business_so_far__c,business_this_month__c,meets_attended__c,psm__c,phone,potential__c,status__c,title,email,createddate`; 
            var UUID_Contact = uuidv4();
            var createdDate = dtUtil.todayDatetime();
            contactFieldsValues =[contactObj.firstname,contactObj.lastname,UUID_Contact, contactObj.asm__c, contactObj.area__c, contactObj.attached_dealer__c, contactObj.business_so_far__c, contactObj.business_this_month__c, contactObj.meets_attended__c, contactObj.psm__c, contactObj.phone, contactObj.potential__c, contactObj.status__c, contactObj.title,contactObj.email, createdDate];
